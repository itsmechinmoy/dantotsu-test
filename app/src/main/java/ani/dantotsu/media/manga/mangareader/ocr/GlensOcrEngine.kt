...
data class GlensPageResult(
    val text: String,
    val regions: List<OcrRegion>,
)

class GlensOcrEngine {
    private val textPostprocessor = TextPostprocessor()

    suspend fun recognizePage(image: Bitmap): GlensPageResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.i("GlensOcrEngine", "OCR(glens) Runtime: recognizePage total time: $totalTime ms")
            pageResult
        } catch (e: Exception) {
            Log.e("GlensOcrEngine", "OCR (glens) page recognition failed", e)
            throw e
        }
    }
...
private class TextPostprocessor {
